...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall qaicli and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g qaicli",
      pnpm: "pnpm uninstall -g qaicli",
      bun: "bun remove -g qaicli",
      yarn: "yarn global remove qaicli",
      brew: "brew uninstall qaicli",
      choco: "choco uninstall qaicli",
      scoop: "scoop uninstall qaicli",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall qaicli -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using QAI!")
}
